...
int main()
{
    ...
    while (i > 0 && SEARCH_KEYS) {
        ...
        if (hm_search(hm, key_buffer, &val) != HM_SUCCESS) {
            ...
            // __FUNCTION__, __LINE__, key_buffer, "null");
        }

        // printf("[%s][%s](%d) - hm_search: k:[%s] v:[%s]\n", __FILE__,
        // __FUNCTION__, __LINE__, key_buffer, (char*)val);
        i--;
    }
    end = clock();
    time_spent = (double)(end - start) / CLOCKS_PER_SEC * 1000.0;
    ...
    // printf(
    //     "Time taken to insert: %.2f ms [sbmps: %d | sbkys: %d | rate: %.2f
    //     ins/s]\n", time_spent, MAX_SUBMAPS, MAX_TEST_SUBMAP_INSERT_KEYS,
    ...
    for (i = 0; i < MAX_SUBMAPS; i++) {
        ...
        if (hm_search(hm, submap_buffer, &val) != HM_SUCCESS) {
            printf("[%s][%s](%d) - hm_search: k:[%s] v:[%s]\n", __FILE__, __FUNCTION__, __LINE__, submap_buffer,
                "null");
            continue;
        }
    ...
    // printf(
    //     "Time taken to search: %.2f ms [submps: %d | subkys: %d | rate: %.2f
    //     lkps/s]\n", time_spent, MAX_SUBMAPS, MAX_TEST_INSERT_KEYS,
    //     MAX_TEST_INSERT_KEYS/time_spent*1000.0
    //);
